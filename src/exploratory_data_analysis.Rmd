...
train_imputed_reduced = train_imputed.iloc[:, 1:]
train_imputed_scaled = scaler.fit_transform(train_imputed_reduced)

# perform pca
pca = PCA(n_components=3)
train_imputed_pca = pca.fit_transform(train_imputed_scaled)
...
For further analysis and modeling, I stored three versions of the train data: the first is the original train data, the second is 'train_complete_cases' obtained by deleting rows with missing values (210 rows), and 'train_imputed' where missing values are replaced with predictions from the Random Forest classifier. To ensure the best predictions for our project, we can use all three datasets and compare their performances. It is worth mentioning that some regressors do not accept missing values in the data at all, so we might need to use either 'train_complete_cases' or 'train_imputed' at some point.

I also performed a preliminary PCA, an unsupervised machine learning method, to further explore patterns in the data. Since PCA does not work on data with NaNs, I used the 'train_imputed' data for this task. Specifying 3 principal components, the results show that PC1, PC2, and PC3 explain 16%, 12%, and 10% of the variance in the data, respectively. The PCA analysis will be completed by the next team meeting.
